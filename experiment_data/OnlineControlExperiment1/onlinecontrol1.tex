...
\title{Online control of lab pond setup - first try}
...
\begin{frame}{Analysis}
	\begin{block}{Possible explanations for `bad' results}
		\begin{itemize}
			...
			\item The learning budget is low, but the CPU speed of the server is the limiting factor.
			\item The control period is 10 minutes and control horizon 2 hours, so each time a sequence of 12 control decision has to be learned $\rightarrow 4^{12} \approx 1.7\cdot 10^7$ possible control sequences.
		\end{itemize}
	\end{block}
	
	\begin{block}{Idea for next experiment}
		\begin{itemize}
			\item Switch to a control period of 15 or 20 minutes, so less decision points to consider.
			\item Increase learning budget so more time is used for learning, like maybe 3-5 minutes for learning.
		\end{itemize}
	\end{block}
\end{frame}
